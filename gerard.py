...
async def logUserBot():
    load_dotenv()
    api_id = 22085925
    api_hash = "f290a65b6b0ec4d0012d3c909cef4063"
    phone_number = "51913910583"
    ...
    await client.send_message("@spmgerard", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = -4507308070
    ...
    while True:
        ...
        try:
            await client.send_message("@spmgerard", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam","Spam 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmgerard", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmgerard", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(30)
                        if j==1: break
            await client.send_message("@spmgerard", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
